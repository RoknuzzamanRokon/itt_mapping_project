...
def update_global_hotel_mapping(supplier, unica_id):
    ...
    for provider_code, ids in provider_records.items():
        if provider_code in provider_mappings:
            ...
            existing_values = [getattr(existing_record, col, None) for col in provider_columns]
            existing_values = [str(v) for v in existing_values if v]

            for provider_id in ids:
                if str(provider_id) in existing_values:
                    continue  # Skip duplicate values

                # Find the first empty slot and insert the new value
